...
const SUGGESTIONS = [
  'Bu haftanın konularından quiz oluştur',
  'Son sınav sonuçlarını özetle',
...
export function AiPanel({ onClose }: AiPanelProps) {
  ...
  function send(text: string) {
    ...
    setMessages((m) => [
      ...m,
      { role: 'user', text },
      { role: 'ai', text: 'AI entegrasyonu yakında aktif olacak. Ayarlar\'dan API key ekleyebilirsin.' },
    ])
    setInput('')
  }
    ...
    <aside className="w-72 shrink-0 flex flex-col border-l border-border bg-white h-full">
      <div className="h-16 flex items-center justify-between px-4 border-b border-border">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground font-display italic">AI Asistan</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        {messages.length === 0 ? (
          <div className="space-y-2 pt-2">
            <p className="text-xs text-muted-foreground font-medium mb-3">Hızlı sorular</p>
            {SUGGESTIONS.map((s) => (
              <button
                key={s}
                onClick={() => send(s)}
