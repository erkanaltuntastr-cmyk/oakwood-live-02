...
export function Subjects() {
  ...
  if (!activeChildId || !child) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-2 text-muted-foreground">
        <p className="text-lg font-display italic">Dersler</p>
  ...
  function toggleSubject(name: string) {
    const updated = childSubjects.map((s) =>
      s.name === name ? { ...s, active: !s.active } : s
    )
    updateProfile(child!.id, { subjects: updated })
  }

  function addSubject(name: string) {
    updateProfile(child!.id, {
      subjects: [...childSubjects, { name, active: true }],
    })
  }

  function removeSubject(name: string) {
    updateProfile(child!.id, {
      subjects: childSubjects.filter((s) => s.name !== name),
    })
  }
  ...
  return (
    <div className="max-w-2xl space-y-8">
      <div>
        <h1 className="text-2xl font-display font-semibold text-foreground italic">Dersler</h1>
        <p className="text-muted-foreground mt-1 text-sm">
          {child.name}'in aktif dersleri. Aktif dersler sınavlarda ve raporlarda görünür.
        </p>
      </div>

      {/* Active subjects */}
      <section className="space-y-3">
        <h2 className="text-sm font-semibold text-foreground">Aktif Dersler</h2>
        {childSubjects.length === 0 && (
          <div className="oak-card p-6 text-center text-muted-foreground text-sm">
        ...
        <div className="grid gap-2">
          {childSubjects.map(({ name, active }) => (
            <div key={name} className={cn(
              'oak-card flex items-center justify-between px-4 py-3 transition-opacity',
              !active && 'opacity-50'
            )}>
      ...
      {availableToAdd.length > 0 && (
        <section className="space-y-3">
          <h2 className="text-sm font-semibold text-muted-foreground">Ders Ekle</h2>
