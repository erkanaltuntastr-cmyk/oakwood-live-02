...
function ChildCard({ childId, isActive, onSelect }: { childId: string; isActive: boolean; onSelect: () => void }) {
  ...
  return (
    <button
      onClick={onSelect}
      className={cn(
        'w-full text-left rounded-2xl border p-5 transition-all duration-150 hover:shadow-card-md group',
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {isActive && <span className="text-xs text-primary font-medium">Seçili</span>}
          </div>
        </div>
...
export function ChildrenHub() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div>
        <h1 className="text-2xl font-display font-semibold text-foreground italic">Çocuklar</h1>
      ...
      {childIds.length === 0 ? (
        <div className="oak-card p-10 text-center text-muted-foreground text-sm">
          Henüz çocuk eklenmemiş. Ayarlar'dan ekleyebilirsin.
        </div>
      ) : (
