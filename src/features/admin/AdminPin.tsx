    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-8">
      ...
      <div className="flex flex-col items-center gap-8 w-full max-w-xs">
        <div className="text-center">
          ...
          <p className="font-semibold text-lg text-foreground">Admin Girişi</p>
          <p className="text-sm text-muted-foreground mt-0.5">PIN'ini gir</p>
        </div>

        <div className={cn('flex gap-4', shake && 'animate-bounce')}>
          {dots.map((filled, i) => (
            <div key={i} className={cn('w-4 h-4 rounded-full border-2 transition-all', filled ? 'bg-primary border-primary scale-110' : 'border-muted-foreground')} />
          ))}
        </div>

        {isLocked && <p className="text-destructive text-sm">Kilitli. {remaining}s bekle.</p>}
        {!isLocked && attempts > 0 && attempts < 5 && (
          <p className="text-destructive text-sm">Yanlış PIN. {5 - attempts} hakkın kaldı.</p>
        )}
